...
namespace Plugins\OAuth2\App\Grant;
...
class AuthCodeGrant extends AbstractAuthorizeGrant
{
	...
	public function validateAuthorizationRequest(ServerRequestInterface $request)
	{
		...
		if (empty($redirectUri) || (is_array($client->getRedirectUri()) && count($client->getRedirectUri()) !== 1) || empty($client->getRedirectUri()))
		{
			$this->getEmitter()->emit(new RequestEvent(RequestEvent::CLIENT_AUTHENTICATION_FAILED, $request));
			throw OAuthServerException::invalidRequest('redirect_uri');
		} else {

			$url = is_array($client->getRedirectUri()) ? array_values($client->getRedirectUri())[0] : $client->getRedirectUri();

			if (strncmp($redirectUri, $url, strlen($url)) !== 0)
			{
				$this->getEmitter()->emit(new RequestEvent(RequestEvent::CLIENT_AUTHENTICATION_FAILED, $request));
				throw OAuthServerException::invalidRequest('redirect_uri');
			}
