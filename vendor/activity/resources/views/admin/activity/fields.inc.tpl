...
<div class="form-group">
	...
	<div class="col-md-9">
		<select name="type_id" id="type_id" class="form-control select-model" value="<{if $_data->type_id}><{$_data->type_id}><{else}>0<{/if}>" data-model="admin/activity-type" data-text="{name}" data-placeholder="请选择活动类型"></select>
	</div>
</div>
...
(function($){
	$().ready(function(){
		...
		$('#type_id').on('change',function(){
			$.post("<{'admin/activity/getTypeHtml'|url}>", { id: "<{$_data.id}>", type_id: $(this).val(),of:'html'},
			  function(data){
