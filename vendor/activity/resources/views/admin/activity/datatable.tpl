...
<{block "filter"}>
<{include file="admin/activity/filters.inc.tpl"}>
<{/block}>
...
var columns_plus = [
	{'data': 'admin', orderable: false, 'render': function(data, type, full){
	...
	{'data': 'status',render:function(data, type, full){
	    ...
	    switch(parseInt(data)){
	        case 0 : status_tag='<a href="<{'admin/activity/setShelves'|url}>/'+full['id']+'/1" title="上架">未上架</a>';break;
	        case 1 :status_tag='<a href="<{'admin/activity/setShelves'|url}>/'+full['id']+'/-1" title="下架">上架中</a>'; break;
	        case -1 :status_tag='<a href="<{'admin/activity/setShelves'|url}>/'+full['id']+'/1" title="上架">下架</a>'; break;
...
var columns_options_plus = [
	'<a href="<{'admin/product?'|url}>base=&filters[activity_type][in][]='+full['id']+'" data-toggle="tooltip" title="商品列表" class="btn btn-xs btn-info"><i class="fa fa-photo"></i></a>'
];
<{/block}>
